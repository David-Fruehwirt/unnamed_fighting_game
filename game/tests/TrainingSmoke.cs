...
public partial class TrainingSmoke : Node
{
    ...
    public override async void _Ready()
    {
        try
        {
            ...
            Check(_dummy.Health==100&&_dummy.IsOnFloor(),"Dummy starts at full health on the deck");
            ...
            Check(_dummy.Health==100,"Jab startup does not damage the dummy");
            await Frames(2);
            Check(_dummy.Health==90&&_dummy.LastDamage==10&&_dummy.HitCount==1,"Jab impact deals exactly 10 damage");
            Check(_dummy.GetNode("DamageNumbers").GetChildCount()==1,"A damage number appears on the hit");
            await Capture("training-hit");
            await Frames(10);
            Check(_dummy.Health==90&&_dummy.HitCount==1,"Several overlapping impact ticks deal damage only once");
            ...
            Check(_dummy.Health==75&&_dummy.HitCount==2&&_dummy.LastDamage==15,"Jab-cross combo deals 10 plus 15, once per punch");
            await Setup(480);await Press();await Frames(16);
            Check(_dummy.Health==100,"A distant punch misses");
            await Setup(670);await Press();await Frames(16);
            Check(_dummy.Health==100,"Punching away from the dummy misses");
            Input.ActionPress("move_left");await Press();Input.ActionRelease("move_left");await Frames(18);
            Check(_dummy.Health==85,"Left-facing cross uses its mirrored fist hit position");
            await Setup(480);await Press();await Frames(7);_player.Position=new(570,334);await Frames(5);
            Check(_dummy.Health==100,"Entering range during recovery cannot hit");
            await Setup();_player.Position=new(570,310);_player.Velocity=new(0,200);await Frames(1);
            await Press();await Frames(10);
            Check(_dummy.Health==90,"Falling punch can hit and land normally");
            await Setup();_player.Position=new(570,150);await Frames(1);await Press();await Frames(12);
            Check(_dummy.Health==100,"An aerial punch above the target misses");
            await Setup();Input.ActionPress("attack");await Frames(60);Input.ActionRelease("attack");
            Check(_dummy.Health==90&&_dummy.HitCount==1,"Holding J cannot repeatedly damage the target");
            await Setup();await Press();_player.Reset();await Frames(15);
            Check(_dummy.Health==100,"Reset before impact cancels damage");
            ...
            await Frames(120);
            Check(_dummy.Position.DistanceTo(home)<.1&&_dummy.Health==90,"Automatic return restores position after inactivity without healing");
            _dummy.AutoResetPosition=false;_dummy.TakeHit(10,"Jab",new(-120,-90));await Frames(135);
            Check(_dummy.Position.X<home.X-3&&_dummy.Health==80,"Automatic return off leaves the displaced dummy in place");
            ...
            _dummy.TakeHit(10,"Jab",new(120,-90));await Frames(70);
            _dummy.TakeHit(10,"Jab",new(120,-90));await Frames(70);
            Check(_dummy.Position.X>home.X+2,"New hits restart the two-second return timer");
            _dummy.AutoResetPosition=false;_dummy.Position=new(800,710);await Frames(2);
            Check(_dummy.Position.DistanceTo(home)<.1&&_dummy.Health==100,"Off-stage rescue works even with automatic return disabled");
            _dummy.KnockbackEnabled=false;
            _dummy.TakeHit(95,"Test",Vector2.Zero);
            Check(_dummy.TakeHit(15,"Cross",Vector2.Zero)==5&&_dummy.Health==0&&_dummy.LastDamage==5,"Lethal hit clamps HP and displays actual damage removed");
            Check(_dummy.TakeHit(10,"Jab",Vector2.Zero)==0,"Empty-health dummy ignores hits while refilling");
            await Frames(62);
            Check(_dummy.Health==100&&!_dummy.Recovering,"Depleted health refills after one second");
            ...
            _dummy.TakeHit(10,"Test",Vector2.Zero);_dummy.Position+=new Vector2(30,0);
            ((Button)_settings.FindChild("ResetDummy",true,false)).EmitSignal(Button.SignalName.Pressed);
            Check(_dummy.Position==home&&_dummy.Health==100&&_dummy.LastDamage==0,"Reset Dummy button immediately restores position and health while paused");
