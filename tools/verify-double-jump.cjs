...
const base='1b43084'; const old=p=>JSON.parse(cp.execFileSync('git',['show',base+':'+p],{maxBuffer:100*1024*1024}));
...
const poses=current('art/poses.json').frames,oldposes=old('art/poses.json').frames;assert.equal(poses.length,45);assert.deepStrictEqual(poses.slice(0,37),oldposes);
let maxError=0;
for(const p of poses.slice(37))for(const [id,bone] of Object.entries(p.parts)){
...
for(const name of fs.readdirSync('art/exports/layers').filter(n=>n.endsWith('.png'))){
 ...
 assert.equal(n.width,45*128);for(let y=0;y<128;y++)assert(o.data.subarray(y*o.width*4,(y+1)*o.width*4).equals(n.data.subarray(y*n.width*4,y*n.width*4+o.width*4)),name);
...
cp.execFileSync('git',['diff','--exit-code',base,'--','art/draw','art/parts','art/palette.json','art/stages','game/assets/stages','art/fight-effects.pixel.json','art/cross-effects.pixel.json']);
...
console.log(JSON.stringify({protectedPartFrames:592,bodyAtlasFrames:45,maxBoneLengthRoundingError:maxError,oldClipsUnchanged:true,rgbaIdentity:true},null,2));
