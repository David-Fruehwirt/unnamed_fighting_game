...
{
    static void VerifyFight()
    {
        const string baseline="404ac55";
        ...
        foreach(string clip in new[]{"idle","run","prepare","jump","fall","land","jump_sequence"})paths.AddRange(new[]{
        ...
        foreach(string clip in new[]{"idle","run","prepare","jump","fall","land","jump_sequence"})
        ...
        if(frames.Keys.Count(id=>id.StartsWith("attack_"))!=5)
            throw new Exception("Attack must contain exactly five poses");
        ...
        for(int i=0;i<24;i++)if(!JsonNode.DeepEquals(poses[i],oldPoses[i]))throw new Exception("Protected attachment pose changed");
        var idle=JsonSerializer.Deserialize<Dictionary<string,Placement>>(poses[0]!["parts"]!.ToJsonString())!;
        double maxError=0;
        foreach(var pose in poses.Skip(24))
        {
            ...
            foreach(string foot in new[]{"near_foot","far_foot"})
                if(parts[foot]!=idle[foot])throw new Exception("Jab foot contact moved");
        ...
        var fx=JsonNode.Parse(File.ReadAllText("art/fight-effects.pixel.json"))!["characters"]![0]!;
        var attack=c["animations"]!.AsArray().Single(a=>a!["id"]!.GetValue<string>()=="attack")!;
        ...
        if(attack["frames"]!.AsArray().Select((f,i)=>f!["durationTicks"]!.GetValue<int>()==FightTicks[i]).Any(ok=>!ok))
        ...
        for(const name of fs.readdirSync('art/exports/layers').filter(n=>n.endsWith('.png'))){
            ...
            if(now.width!==29*128||now.height!==128)throw Error('Invalid atlas');
            const a=[],b=[];
            for(let y=0;y<128;y++){
                a.push(old.data.subarray(y*old.width*4,(y*old.width+24*128)*4));
                b.push(now.data.subarray(y*now.width*4,(y*now.width+24*128)*4));
            }
        ...
        const effects=['art/exports/effects/attack.png','art/pixelloid/effects/attack.png','game/assets/soldier_effects/attack.png']
            .map(p=>PNG.sync.read(fs.readFileSync(p)));
        if(effects.some(p=>p.width!==800||p.height!==128||sha(p.data)!==sha(effects[0].data)))throw Error('Effect import mismatch');
        const counts=[];
        for(let f=0;f<5;f++){
            let count=0;for(let y=0;y<128;y++)for(let x=0;x<160;x++){
                const alpha=effects[0].data[(y*800+f*160+x)*4+3];
                if(alpha){count++;if(x===0||x===159||y===0||y===127)throw Error('Clipped effect');}
            }
            if(!count)throw Error('Empty effect phase');counts.push(count);
        }
        fs.writeFileSync('art/FIGHT_VERIFICATION.json',JSON.stringify({baseline,protectedClips:['idle','run','prepare','jump','fall','land','jump_sequence'],
            protectedPartFrames:384,attackFrames:5,durationMs:300,maximumBoneRoundingPixels:Number(process.argv[3]),
            plantedFeet:true,effectRgbaSha256:sha(effects[0].data),effectPixelCounts:counts,parts},null,2)+'\n');
        console.log('PASS: 384 idle/walk/jump part frames unchanged; Pixelloid and game imports match.');
        """;
        ...
        Console.WriteLine($"PASS: exactly five attack poses in bounds; maximum bone-length rounding {maxError:F3} px.");
    }
}
