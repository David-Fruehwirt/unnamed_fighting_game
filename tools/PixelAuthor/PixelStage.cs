...
public static partial class Program
{
    ...
    {
        const string master="art/stages/stage_1.pixel.json",file="art/stages/stage_1.matched.pixel.json";
        ...
        const settings={pixelSize:1.5,offsetX:0,offsetY:0,samplingMode:'medoid'};
        const result=pixelizeBuffer({width:900,height:450,data:new Uint8ClampedArray(input.data)},settings);
        if(result.width!==600||result.height!==300||result.passthrough)throw Error('Pixelizer did not reduce the stage');
        ...
        fs.writeFileSync('art/stages/STAGE_1_PIXELIZATION.json',JSON.stringify({tool:'Pixelloid',
            ...
            output:[600,300],displayScale:1,passthrough:false,inputRgbaSha256:sha(input.data),outputRgbaSha256:sha(pixels)},null,2)+'\n');
        """);
        byte[] rgba=MatchStagePalette(File.ReadAllBytes("art/work/stage-low.rgba"));
        var report=JsonNode.Parse(File.ReadAllText("art/stages/STAGE_1_PIXELIZATION.json"))!;
        report["quantization"]=JsonSerializer.SerializeToNode(new{method="nearest existing Stage 1 palette",opaqueColors=32,dithering=false});
        ...
        var palette=new Dictionary<string,string>();var cells=new string?[600*300];
        ...
        if(!File.Exists(file))
        {
            var project=Project("stage_pixel",600,300,["stage"]);
        ...
        ops.Add(new{type="addFrame",characterId="stage_pixel",viewId="right",frame=new{id="stage",name="Pixel scrapyard",durationTicks=1,
            cels=new{stage=new{grid=new{width=600,height=300,cells},offset=new{x=0,y=0}}}}});
        ...
        RunPix("render",file,"--frame","stage","--scale","1","--out","art/exports/stages/stage_1.png");
        RunPix("render",file,"--frame","stage","--scale","1","--out","art/previews/stage_1.png");
        ...
        Save("art/stages/stage_1.layout.json",new{reference="reference_pics/stage_1.jpg",
            ...
            notes="Display-pixel coordinates: flat invisible surface at reference row 294 across the widest deck section. Stage artwork is 600x300 logical pixels exported at 1x to match the Soldier."});
        File.WriteAllText(file,JsonNode.Parse(File.ReadAllText(file))!.ToJsonString()+"\n");
    }

    static void VerifyPixelStage()
    {
        Console.WriteLine(RunPix("validate","art/stages/stage_1.matched.pixel.json","--json"));
        StageBridge("""
        const fs=require('fs'),path=require('path'),cp=require('child_process'),crypto=require('crypto');
        const {PNG}=require(path.join(path.dirname(process.argv[1]),'../../../node_modules/pngjs'));
        cp.execFileSync('git',['diff','--exit-code','f02df82','--','art/parts','art/draw','art/palette.json',
            'art/soldier.pixel.json','art/poses.json','art/fight-effects.pixel.json','art/exports/layers','art/exports/animations',
            'art/exports/effects','game/assets/soldier_frames','game/assets/soldier_effects','game/scenes/soldier.tscn','game/scripts/Soldier.cs',
        ...
        if(low.width!==600||low.height!==300)throw Error('Stage logical resolution must match display resolution');
        const pixelization=JSON.parse(fs.readFileSync('art/stages/STAGE_1_PIXELIZATION.json'));
        if(pixelization.passthrough||pixelization.settings.pixelSize!==1.5||sha(low.data)!==pixelization.quantizedRgbaSha256)throw Error('Missing real pixelization');
        for(let y=0;y<300;y++)for(let x=0;x<600;x++){
            const i=(y*600+x)*4,j=i;
            for(let c=0;c<4;c++)if(img.data[i+c]!==low.data[j+c])throw Error('Stage pixels were scaled');
        ...
        fs.writeFileSync('art/stages/STAGE_1_VERIFICATION.json',JSON.stringify({baseline:'f02df82',soldierAssetsUnchanged:true,
            masterPreserved:true,width:600,height:300,logicalWidth:600,logicalHeight:300,pixelBlockSize:1,opaqueColors:32,actualPixelloidReduction:true,
            exportPixelloidGameRgbaSha256:sha(img.data),deckLeft:214,deckRight:762,deckY:334,deckSections:1,sharedLayout:true},null,2)+'\n');
        console.log('PASS: real Pixelloid reduction, 1:1 pixels matching the Soldier, flat deck, matching imports and unchanged character artwork.');
        """);
    }
}
